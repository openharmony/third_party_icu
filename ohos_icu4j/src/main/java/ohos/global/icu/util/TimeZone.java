 ...
 * #
 * ohos.global.icu.util.TimeZone.DefaultTimeZoneType = ICU
 * </pre>
 * </blockquote>
 *
 * <p>This property is included in ICUConfig.properties in ohos.global.icu package.  When the
 * <code>TimeZone</code> class is loaded, the initialization code checks if the property
 * <code>ohos.global.icu.util.TimeZone.DefaultTimeZoneType=xxx</code> is defined by the system
...
abstract public class TimeZone implements Serializable, Cloneable, Freezable<TimeZone> {
    ...
    public static synchronized void setDefault(TimeZone tz) {
        ...
        if (tz != null) {
            ...
            // can interoperate with ohos.global.icu.util classes.
            java.util.TimeZone jdkZone = null;
            if (tz instanceof JavaTimeZone) {
                jdkZone = ((JavaTimeZone)tz).unwrap();
            } else if (tz instanceof ohos.global.icu.impl.OlsonTimeZone) {
